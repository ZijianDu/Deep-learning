...
def extraction(seed):
	...
	train_iterator, test_iterator = load_dataset("questions", vocab_size)
...
def main():
	...
	train_iterator, test_iterator = load_dataset("questions", batch_size)
	...
	with open("trained GRU for questions","wb") as fid:
		pickle.dump(model,fid)
	
	
	with open("trained GRU for questions","rb") as fid:
	...
	for batch in test_iterator:
		...
		_, indices = torch.max(spam_space, 1)
		#print(indices.size())
		...
		for i in range(batch_size):
			print(list(indices.data.numpy()))
			print(list(label.data.numpy()))	
			if indices.data.numpy()[i]==label.data.numpy()[i]:
				correct +=1
			counter+=1
	print("accuracy is:")
