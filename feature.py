...
# define class to load the dataset

def places(seed):
    np.random.seed(seed)
    preprocessor = transforms.Compose([resize, transforms.ToTensor(), normalize, ])
    # create image folder
    places_train = torchvision.datasets.ImageFolder('./places/train',
                                                    transform=preprocessor)
    places_train.train_labels = []
    # save the labels for train instances
    for i in range(places_train.__len__()):
        places_train.train_labels.append(places_train.imgs[i][1])
    # create loader class to load formatted data
    train_loader = torch.utils.data.DataLoader(places_train, batch_size=4, shuffle=True, num_workers=1, pin_memory=True)
    # create test folder
    places_test = torchvision.datasets.ImageFolder('./places/test',
                                                   transform=preprocessor)
    ...
    # test_loader
    test_loader = torch.utils.data.DataLoader(places_test, batch_size=1, shuffle=True, num_workers=1, pin_memory=True)
...
# define preprocessor
preprocessor = transforms.Compose([resize, transforms.ToTensor(), normalize, ])
train_loader, test_loader = places(2)
...
# test trained SVM classifier using test data
...
def main_vgg():
    ...
    # vgg16 has classifier
    new_classifier = nn.Sequential(*list(pretrained_model.classifier.children())[:-1])
    # update the model classifier
    pretrained_model.classifier = new_classifier
    # train the SVM and save the classifier
    #clf_vgg = train(pretrained_model, 4096)
    #with open("trained SVM using vgg16", 'wb') as fid:
    #    pickle.dump(clf_vgg, fid)

    # load the trained classifier and test
    with open("trained SVM using vgg16", 'rb') as fid:
        clf_vgg = pickle.load(fid)
    # test the SVM and obtain results
    targets, prediction = test(pretrained_model, 4096, clf_vgg)
    plot_confusion_matrix(prediction, targets)
...
def main_inception():
    ...
    # train the model
    clf_inceptionv3 = train(pretrained_model, 2048)
    with open("trained SVM using inception", "wb") as fid:
        pickle.dump(clf_inceptionv3, fid)

    # load the trained classifier and test
    with open("trained SVM using inception", 'rb') as fid:
        clf_inceptionv3 = pickle.load(fid)

    # test the SVM and plot the confusion matrix
    targets, prediction = test(pretrained_model, 2048, clf_inceptionv3)
    plot_confusion_matrix(prediction, targets)
